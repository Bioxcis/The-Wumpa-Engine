...
class GameplayChangersSubstate extends MusicBeatSubstate
{
	...
	function getOptions()
	{
		...
		var option:GameplayOption = new GameplayOption('Quantia Ganho de Vida', 'healthgain', 'float', 1);
		...
		var option:GameplayOption = new GameplayOption('Quantia Perda de Vida', 'healthloss', 'float', 1);
		option.scrollSpeed = 2.5;
		option.minValue = 0.5;
		option.maxValue = 5;
		option.changeValue = 0.1;
		...
		var option:GameplayOption = new GameplayOption('InstaKill ao Errar', 'instakill', 'bool', false);
		optionsArray.push(option);

		var option:GameplayOption = new GameplayOption('Modo Pratico', 'practice', 'bool', false);
		optionsArray.push(option);

		var option:GameplayOption = new GameplayOption('Botplay', 'botplay', 'bool', false);
		optionsArray.push(option);

		var option:GameplayOption = new GameplayOption('Notas Espelho', 'mirrorcharts', 'bool', false);
		optionsArray.push(option);

		var option:GameplayOption = new GameplayOption('Notas Aleatorias', 'randomcharts', 'bool', false);
		optionsArray.push(option);
	}
		...
		if(nextAccept <= 0)
		{
						...
						if(pressed) {
							...
							switch(curOption.type)
							{
								...
								case 'string':
									...
									{
										var oOption:GameplayOption = getOptionByName("Tipo Scroll");
										if (oOption != null)
										{
											if (curOption.getValue() == "constante")
											{
			...
			if(controls.RESET)
			{
				for (i in 0...optionsArray.length)
				{
					...
					if(leOption.name == 'Velocidade Scroll')
					{
